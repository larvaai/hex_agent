...
describe("adapter seam", () => {
  ...
  it("adapter_backoff_escalates_when_stream_opens_then_immediately_closes", () => {
    // The fake server returns a valid 200 then drains+closes; without a stability gate the
    // adapter's onopen reset the backoff every cycle, pinning reconnects at 250ms (~4/s storm).
    vi.useFakeTimers();
    const handle = openStream(() => {});
    const es1 = MockEventSource.instances[0];
    es1.onopen?.({}); // 200 received...
    es1.fail(); // ...closed immediately, before it could prove stable → attempt must NOT reset
    vi.advanceTimersByTime(250); // first backoff
    const es2 = MockEventSource.instances[1];
    expect(es2).toBeDefined();
    es2.onopen?.({});
    es2.fail();
    // attempt is now 2 (open never proved stable) → backoff is 500ms, so 250ms is NOT enough
    vi.advanceTimersByTime(250);
    expect(MockEventSource.instances[2]).toBeUndefined(); // still backing off — no storm
    vi.advanceTimersByTime(250);
    expect(MockEventSource.instances[2]).toBeDefined(); // reconnects only after the escalated 500ms
    handle.close();
    vi.useRealTimers();
  });

  it("adapter_resync_drops_stale_cursor_to_break_resync_loop", () => {
    // out-of-ring resync: if the adapter keeps the stale lastEventId, every reconnect re-triggers
    // resync forever. It must drop the cursor so the reconnect re-syncs from the live ring.
    vi.useFakeTimers();
    const onResync = vi.fn();
    const handle = openStream(() => {}, { lastEventId: "5", onResync });
    const es1 = MockEventSource.instances[0];
    expect(es1.url).toContain("lastEventId=5");
    es1.emit("resync", {}); // server: your cursor fell off the ring
    expect(onResync).toHaveBeenCalledOnce();
    es1.fail(); // server closed the resync stream
    vi.advanceTimersByTime(250);
    const es2 = MockEventSource.instances[1];
    expect(es2).toBeDefined();
    expect(es2.url).not.toContain("lastEventId"); // cursor dropped → no infinite resync loop
    handle.close();
    vi.useRealTimers();
  });
