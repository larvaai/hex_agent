# E21 Control Plane UI (fake-backend slice)

A React + Vite + TypeScript UI for the E21 realtime control plane, built **against a fake
Python backend that speaks the real contract**. The fake (`tools/fake_control_server.py`)
reuses `control/` — the same `Redactor`, `SessionSeq`, `parse_command`, registries, and
`build_snapshot` the real backend will — so wiring the real backend is **"change the URL"**,
not "re-render".
