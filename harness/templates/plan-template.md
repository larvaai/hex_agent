...
> không hiển nhiên. Tag `[UNVERIFIED]` lên claim chưa có anchor.
...
- [ ] Mỗi phase red→green TDD; `python3 -m pytest harness/tests/ -q` xanh sau mỗi phase.
- [ ] `verify_install.py --strict` sạch sau rebuild manifest.
- [ ] TBD — tiêu chí nghiệm thu đặc thù của plan này.

## Rollback
TBD — mỗi phase commit riêng; cách hoàn tác (git revert <range> + rebuild manifest).
