...
def check_docker_daemon(run=subprocess.run, docker_bin="docker"):
    rc, err = _docker_info(run, docker_bin)
    if rc == 0:
...
def check_docker_perm(run=subprocess.run, docker_bin="docker"):
    rc, err = _docker_info(run, docker_bin)
    if "permission denied" in err.lower():
        return Finding(
            "docker_perm", "fail",
...
def check_inputs_exist(paths=()):
    ...
    return Finding(
        ...
        "pass existing plan/PRD paths: /hs-flow:afk \"<plan> <prd>\" [N]",
        _FALLBACK)
...
def preflight(plan_path=None, prd_path=None, *, image=DEFAULT_IMAGE,
    ...
    return [
        ...
        _safe("docker_daemon", check_docker_daemon, run, docker_bin),
        _safe("docker_perm", check_docker_perm, run, docker_bin),
