...
class InvocationResult:
    ...
    question: bool = False
    diff_count: int = 0          # changed-file count this iteration (git progress)
...
def _git_diff_count(repo_root) -> int:
    """Changed-file count via `git diff --numstat` (staged+unstaged). 0 on error."""
    try:
        out = subprocess.run(
...
def make_claude_invoker(prompt, repo_root, *, timeout=3600):
    ...
    field; progress is measured by git diff; a `blocked` AFK_STATUS maps to a
    pending question via the controller."""
    ...
    def invoke(iteration, session_id) -> InvocationResult:
        ...
        return InvocationResult(
            stdout=stdout, is_error=is_error, diff_count=_git_diff_count(repo_root),
