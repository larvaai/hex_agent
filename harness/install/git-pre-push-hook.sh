...
# Missing python3 fails CLOSED — a transport gate that silently passes when
...
if ! command -v python3 >/dev/null 2>&1; then
    echo "pre-push: python3 not found — the stage gate cannot run." >&2
    echo "Install python3, or in an emergency remove .git/hooks/pre-push" >&2
    echo "(verify_install --strict reports an installed copy that differs" >&2
    echo "from the source; a removed copy shows up as 'not installed')." >&2
    exit 2
fi
