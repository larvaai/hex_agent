...
the invoke name `hs:plan`. The skill frontmatter keeps `name: hs:plan` for telemetry mapping even
though the plugin loader derives the command from plugin+dir. Telemetry records
may carry either identity (`hs:plan`, `plan`), so lenses resolve everything to
the canonical dir slug through this map. `owned` = dirs whose frontmatter name is
`hs:*` (harness-authored) — "never used" advice is only actionable for those, a
vendored tool sitting unused is expected.
...
# Skills authored by THIS harness carry the hs: prefix or a sibling-plugin
# prefix (hs-viz:, hs-uiux:, ...). A skill vendored in place that kept a foreign
# name (e.g. ck:tool) is NOT owned.
_OWNED_NAME_PREFIX = "hs:"


def _is_owned_name(name) -> bool:
    """True for a skill in the harness plugin family — the `hs` core or a
    sibling plugin (hs-viz, hs-uiux, ...). Namespace before ':' is `hs` or
    `hs-<x>`. A foreign-namespaced vendored skill (ck:tool) is not owned."""
    if not name:
        return False
    ns = name.split(":", 1)[0]
...
def _frontmatter(skill_md: Path) -> str:
    """The YAML frontmatter block (text before the closing '---'). Body prose
    that merely documents tier values is excluded, so it can't false-positive."""
    try:
...
def tier_problems(sdir=None) -> list:
    """'dir: reason' for every harness-owned (hs / hs-viz / ...) skill whose
    frontmatter compliance-tier is missing or outside VALID_TIERS. Vendored
    (foreign-namespaced) dirs are exempt — their tier is not the harness's to
    dictate. No `sdir` scans the whole plugin family; an explicit `sdir` scans
    just that one dir."""
    sources = [Path(sdir)] if sdir is not None else skills_dirs()
    problems = []
    for src in sources:
