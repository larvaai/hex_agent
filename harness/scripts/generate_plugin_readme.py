...
def _frontmatter(skill_md):
    m = _FM.match(skill_md.read_text(encoding="utf-8"))
    if not m:
        return {}
    try:
        import yaml
...
def _skills(plug_dir):
    ...
    for sk in sorted((plug_dir / "skills").rglob("SKILL.md")) if (plug_dir / "skills").is_dir() else []:
        ...
        out.append((str(name), _first_sentence(fm.get("description", ""))))
...
def main(argv=None) -> int:
    ...
    for plug in _iter_plugins(root):
        if args.check:
            r = plug / "README.md"
            if not r.is_file() or (MARKER in r.read_text(encoding="utf-8")
                                   and r.read_text(encoding="utf-8") != render_plugin_readme(plug)):
