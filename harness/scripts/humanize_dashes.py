...
def _transform_line(line: str, repl: str) -> str:
    ...
    for m in _CODE_SPAN_RE.finditer(line):
        out.append(_transform_segment(line[last:m.start()], repl))
        out.append(m.group(0))  # code span verbatim
        last = m.end()
    out.append(_transform_segment(line[last:], repl))
    return "".join(out)
