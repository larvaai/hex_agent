#!/usr/bin/env python3
"""findings_store.py — durable belief store for hs-flow:compound (append-only + replay-on-read).

hs-flow:compound stops at Propose and writes nothing durable ("The report is the whole job"),
...
def _default_path() -> Path:
    ...
    try:
        ...
        return Path("harness/state/findings.jsonl")
...
def _belief_from(obs: dict, reinforces: List[dict], now: datetime) -> dict:
    ...
    return {
        "id": obs.get("id"),
        "text": obs.get("text"),
        "text_canon": obs.get("text_canon", canon(obs.get("text", ""))),
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(description="Durable belief store for hs-flow:compound.")
