...
both REUSE an existing enable path rather than inventing a parallel flag:
  ...
  * PLUGIN-bearing components (the sibling plugins `hs-<group>`) write
    `enabledPlugins["hs-<group>@hs-local"]: false` into the target settings.json
    — a real skill-disable: Claude Code stops loading that plugin's skills.
...
For HOOK-bearing components the load-bearing effect is the hook enable flag, and
SKILL/SCRIPT/DATA membership is otherwise informational. For PLUGIN-bearing
components the `enabledPlugins` toggle IS the skill-disable — there, skill
membership is load-bearing (the plugin-only components carry no hooks at all).

Mirrors team_config.py: validate BEFORE any write (an invalid toggle raises and
...
_HOOKS_DEFAULT = _HERE.parent / "hooks" / "harness-hooks.yaml"
...
# ---- writers (mirror team_config: header-preserving, validate-first) ------
...
def _write_enabled_plugins(settings_path, states, *,
    ...
    No-op when `states` is empty — a settings file is never created just to
    write nothing (a component with no plugin must not spawn enabledPlugins)."""
    import json
    if not states:
        return
