...
Three subcommands:
  ...
  set-preset <strict|balanced|lenient>   change the baseline for all guards.
...
_CATEGORY_ORDER = ("safety", "enforcement", "advisory", "nudge")
...
def _locked_update(path: Path, *, gid=None, mode=None, preset=None):
    ...
    def _apply():
        ...
        if gid is not None:
            old = guard_policy.resolve_mode(gid, path)
