...
def detect_language(text: str) -> str:
    """vi when Vietnamese diacritics are a meaningful share of the letters, else en.

    English is the safe default: empty or diacritic-free text reads as en, so a
    plain English report never raises a false vi alarm.
    """
    alpha = len(_ALPHA_RE.findall(text))
    if not alpha:
        return "en"
    return "vi" if len(_VI_RE.findall(text)) / alpha >= 0.04 else "en"
