...
This module is that one place. A guard resolves its mode from:
  ...
  2. the active `preset` (strict | balanced | lenient) applied to the guard's
...
Loader idiom mirrors team_config (resolve off __file__, typed error, lazy
yaml). Unlike team_config this gate DOES honor an env override
...
_PRESETS = ("strict", "balanced", "lenient")
...
_NUDGE = {"category": "nudge", "floor": None}
...
GUARD_REGISTRY = {
    ...
    "gate_stage": _ENFORCE,
    "ownership_guard": _ENFORCE,
    "ownership_reconcile": _ENFORCE,
    ...
    "ownership_overlap": _ADVISORY,
    # nudge — gentle reminders; ship ASLEEP (off) at the balanced default so a
    # fresh install is quiet, and surface (warn) only when a power user opts
    # into strict. This is the one category whose balanced column is off: it
    # preserves the pre-policy "nudges default OFF" behavior under one engine.
    "memory_gap_hook": _NUDGE,
    "cook_isolation_nudge": _NUDGE,
    "discover_isolation_nudge": _NUDGE,
    "decision_capture_nudge": _NUDGE,
}
...
_PRESET_TABLE = {
    "safety":      {"strict": "block", "balanced": "block", "lenient": "block"},
    "enforcement": {"strict": "block", "balanced": "block", "lenient": "warn"},
    "advisory":    {"strict": "warn",  "balanced": "warn",  "lenient": "off"},
    "nudge":       {"strict": "warn",  "balanced": "off",   "lenient": "off"},
}
...
def load_guard_policy(path=None) -> dict:
    ...
    (safety + enforcement still block; only nudges relax), because a harness
...
def gate(guard_id, reason, *, hook, actor=None, session=None):
    ...
    except GuardPolicyError as e:
        trace_log.append_event(hook, "guard_policy_error", actor=actor,
                               session=session, target=guard_id, note=str(e))
