...
Noise vocabulary is reused from hs-flow:loop (Direction/Noise) so the two skills share one
...
def _safe_int(x, default=0):
    """Coerce to int, falling back on a malformed value (fail-soft sort key)."""
    try:
        return int(x)
...
def read_scores(run_id: str) -> dict:
    ...
    scores = {}
    ordered = sorted(read_records(run_id),
                     key=lambda r: (str(r.get("candidate", "")), _safe_int(r.get("trial", 0))))
    for rec in ordered:
        try:
            scores.setdefault(rec["candidate"], []).append(float(rec["value"]))
        except (TypeError, ValueError):
            continue
    return scores
...
def preflight(candidates, metric_cmd, *, budget_seconds=None, budget_tokens=None,
    ...
    the skill dry-runs it once (exit 0 + one number, same contract as hs-flow:loop)."""
    reasons = []
