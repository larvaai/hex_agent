...
# Output path under harness/standards/ per type.
...
def load_template(target_type: str) -> str:
    """Return the raw template skeleton text for a target type."""
    name = TYPE_TEMPLATE.get(target_type)
    if not name:
        raise ValueError(f"no template for type {target_type!r}; "
                         f"known: {sorted(TYPE_TEMPLATE)}")
...
def main(argv=None) -> int:
    ...
    try:
        ...
        rendered = render(load_template(args.type), values)
        out_rel = OUTPUT_PATH_FOR_TYPE[args.type].format(id=values.get("id", "TBD"))
