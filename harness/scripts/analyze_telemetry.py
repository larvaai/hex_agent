...
OVERVIEW_ORDER = ["workflow", "skill_usage", "risk_flags", "reliability",
                  "subagent_outcomes", "observations"]
...
NOT_MEASURED = [
    ...
    "lenses not yet shipped: session, health, validate, memory, "
    "product_memory (not yet shipped — the JSONL data is retained; the "
    "read-only lens is added later without touching this spine)",
]
...
def _render_one(agg: dict) -> str:
    ...
    mod = available_lenses().get(agg.get("_lens_key", ""), (None, None))[0]
