#!/usr/bin/env python3
"""compound_citation_check.py — re-verify the citations in an hs-flow:compound proposal.

hs-flow:compound proposes harness self-improvements, each grounded in a citation of one of
two kinds:
...
def _load_lens_index(path) -> Optional[Dict[str, int]]:
    """Load a chain -> count map from a JSON file. None when no path is given."""
    if not path:
        return None
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(description="Re-verify citations in an hs-flow:compound proposal.")
