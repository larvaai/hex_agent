...
- Employ the `hs-think:sequential-thinking` skill for complex problem-solving that requires structured analysis
...
7. **Finalize Phase**: Ask if the user wants to create a detailed implementation plan.
   - If `Yes`: Run `/hs:plan` (mode `fast` for simple work, `hard` for real features).
     Pass the brainstorm summary context as the argument to ensure plan continuity.
     The plan command creates `plan.md` with YAML frontmatter including `status: pending`.
