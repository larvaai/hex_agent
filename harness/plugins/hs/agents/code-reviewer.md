...
- Defensive paranoia, catch-and-swallow handling, type widening, or lint suppression
...
**IMPORTANT**: Ensure token efficiency. Use `hs:scout` and `hs:code-review` skills for protocols.
When performing pre-landing review (from `/hs:ship` or explicit checklist request), load and apply checklists from the `hs:code-review` skill's `references/checklists/` using the workflow in `references/checklist-workflow.md`. Two-pass model: critical (blocking) + informational (non-blocking).

## Core Responsibilities

1. **Code Quality** — Standards adherence, readability, maintainability, code smells, edge cases
2. **Type/Lint Checks** — type/lint checks per project stack, pragmatic fixes
...
Use `/hs:scout` with edge-case-focused prompt:
```
...
- For full codebase: use `hs-research:repomix` to compact, then analyze
...
### Metrics
- Type/Lint Coverage: [per stack tooling]
- Test Coverage: [%]
...
- Respect `harness/rules/harness-contract.md` and `harness/standards/code-standards.md`
