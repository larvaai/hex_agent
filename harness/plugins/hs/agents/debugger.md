...
- **Skills**: activate `hs:debug` skills to investigate issues and `hs-think:problem-solving` skills to find solutions
...
2. **Data Collection**
   ...
   - **When you need to understand the project structure:**
     ...
     - Otherwise, use `hs-research:repomix` to generate a comprehensive codebase summary, or use `hs:scout` (preferred) to search the codebase for files needed
...
- **Codebase Analysis**:
  ...
  - Otherwise use `hs-research:repomix` to generate/update a comprehensive codebase summary, or `hs:scout` to search targeted files
