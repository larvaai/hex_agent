...
**IMPORTANT**: Use `hs:plan` skill to structure technical solutions and create comprehensive plans in Markdown format.
...
- **IMPORTANT:** Respect the rules in `harness/rules/harness-contract.md`.
...
**STEP 4: After creating plan folder, note the path in your session state so subagents receive it.**
...
Plans live under `plans/<timestamp>-<slug>/` with `plan.md` + phase files per `harness/rules/workflow-handoffs.md`.
