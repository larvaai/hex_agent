...
Run the full checklist before the ship pipeline continues.
Any FAIL item → STOP, report the reason, do not proceed.
...
```bash
python3 -c "import pathlib,sys; a=next(pathlib.Path('plans').glob('*/artifacts'),None); sys.exit(0 if a and (a/'plan-approval.json').exists() else 1)" || echo "plan-approval: MISSING"
```

Missing artifact → **early warning** (not a block here — gate blocks at push).
Suggestion: run `hs:cook` to generate `verification.json`, `hs:code-review` to generate
`review-decision.json`, approve the plan to generate `plan-approval.json`.
