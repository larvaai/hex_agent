...
Any answer that is vague ("probably", "I think") → `AskUserQuestion` or scout further.
...
Use the `debugger` agent to run hypotheses in parallel (max 3 agents per round).
If 2+ hypotheses fail → escalate to `deep`, ask user before continuing.
