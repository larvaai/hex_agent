...
Used at Step 4. Backing: `harness/rules/tdd-discipline.md` +
...
`verdict` is `PASS` only when **all** checks pass. Verdict BLOCKED →
`harness/hooks/gate_stage.py` blocks push/ship.
...
A regression test proves THIS instance is dead. Before declaring done, ask whether a
guard at a higher layer stops the whole class from recurring — not every fix needs all,
but always consider each:
...
A single targeted guard often retires a recurring bug class more cheaply than N future
one-off fixes. Keep it minimal — one guard at the right layer, not four redundant ones.
