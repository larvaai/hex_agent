...
Scout tasks are **independent** of cook/plan tasks — not parent-child.
Scout finishes first; cook hydrates phase tasks separately afterward.

## TaskCreate error handling

If TaskCreate fails → log a warning and continue scouting without task tracking.
Tasks add observability only — they are not blocking.
