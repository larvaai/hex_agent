...
Hook `harness/install/git-pre-push-hook.sh` protects at the transport layer:
- Scrubs `HARNESS_*` env -> prevents config override from outside
- Calls `artifact_check.check_stage("push")` — missing artifact -> exit 2
- `python3` not found -> fail-closed (does not silently pass)

This gate is defense-in-depth: it operates regardless of whether push is invoked via alias, eval, or wrapper.
