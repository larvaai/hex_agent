...
Delegates to `git-manager` agent. Maximum 4 tool calls.
...
Only push if the user explicitly requested it (`cp`, "commit and push").
Pre-push hook `harness/install/git-pre-push-hook.sh` runs automatically — do not bypass.
