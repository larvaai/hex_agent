...
Template for hs:understand output. Write to `plans/reports/` (temporary) or
`docs/` when `--persist`. Every section must be based on files actually read —
do not describe assumed behavior.
...
### File-key
- Only files with a significant role (entrypoint, gate, schema, config) — do
  not list every file. Use absolute paths.
...
- These unknowns become input for hs:plan or hs-research:research.

## Size check
After writing: `wc -l <map-file>` — if > 300 lines, split subtrees into
separate maps or summarize further. An oversized map defeats the purpose of
comprehension.
