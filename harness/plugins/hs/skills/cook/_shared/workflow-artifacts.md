...
`hs:fix` and `hs:cook` use these JSON artifacts to make review and finalize gates deterministic. Scores are advisory. Approval comes from structured proof plus the validator.

## Artifact Directory

- Plan workflow: `plans/<plan-dir>/reports/harness/`
- No-plan workflow: `plans/reports/harness/<timestamp-slug>/`
- Active pointer: `workflow-artifacts.json` (project root, gitignored)
...
```json
{
  ...
  "touchpoints": ["harness/hooks/gate_stage.py"],
  "publicContracts": ["manual command: python3 harness/hooks/gate_stage.py --stage finalize --artifact-dir <dir>"],
  "blastRadius": ["harness hook execution", "fix/cook review cycle"],
  "scoutSummary": "Existing hooks are zero-dependency Python scripts with config-gated fail-open behavior."
}
```
...
```json
{
  "commands": [
    {
      "command": "pytest harness/tests/ -q",
      "status": "pass",
      "exitCode": 0,
