...
Planning carryover (research, red-team, debate) accumulates in context
and shifts cook's focus — the agent can easily get pulled into old reasoning instead of executing
the finalized plan.
...
`harness/hooks/cook_isolation_nudge.py` detects plan + cook in the same session →
prints a `/clear` suggestion. The hook is telemetry fail-open: silent on error, does not block cook.
