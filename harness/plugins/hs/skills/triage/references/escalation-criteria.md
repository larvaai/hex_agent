...
Triage escalates to `hs:plan` when a fix requires a **design change**,
not merely an implementation fix. Fast-fixing an architectural defect creates new bugs.
...
2. Ask the user via AskUserQuestion with 3 options:
   ...
   - **Call hs-think:problem-solving** — reframe the problem from a different angle.
3. Do not self-decide — this is a mandatory stopping point.

Backing: `harness/rules/workflow-handoffs.md` (fix-loop chain 3).
...
## Escalate to hs-think:problem-solving
...
- Route: `/hs-think:problem-solving` with context from the debug report.
