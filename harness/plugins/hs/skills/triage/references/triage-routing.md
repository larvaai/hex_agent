...
2. Record the exact command (this is the baseline for comparison after the fix)
3. Reproducible → proceed to Step 2 (scout)
4. Not reproducible → load references/defect-repro.md
```

Cannot reproduce after 2 attempts → STOP, load `references/defect-repro.md`,
gather more data from the user (logs, core dumps, CI artifacts).
