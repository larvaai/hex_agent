...
`--fix --reply`: only post the final re-review when the loop converges. On a blocker → still post;
the verdict reflects remaining findings, and the body records the blocker for the human reviewer.
