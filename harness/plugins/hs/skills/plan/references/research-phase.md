...
Goal: build sufficient understanding of the problem surface before finalizing a
direction — prevent the planner from inventing facts.

Skip when: `--fast`, a researcher report already exists, or the task is <20 words
and unambiguous.
...
| Tool | When to use |
|---|---|
| Delegate `researcher` agent | Topics requiring web/docs outside the repo — spawn <=2 in parallel, one aspect per agent |
| `hs-think:sequential-thinking` | Multi-step analysis requiring revision (unclear scope, multiple approaches) |
...
1. **Read harness docs first**: `harness/standards/system-architecture.md` + `harness/standards/code-standards.md` — if missing, prompt the user to load them per `harness/standards/README.md`.
