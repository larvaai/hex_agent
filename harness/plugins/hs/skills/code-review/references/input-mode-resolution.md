...
| `codebase` | Full scan | `hs-research:repomix` compact → analyze entire repo |
...
**Codebase**
```
→ delegate to hs-research:repomix to compact the repo
...
- **> 10 files or security-sensitive**: scout + full checklist workflow
  (load `references/review-dimensions.md` in full)

The gap between actual scope (additions/deletions) and the PR description is its own signal
— a description of "fix typo" paired with a +800-line diff is process slop; flag it immediately.
