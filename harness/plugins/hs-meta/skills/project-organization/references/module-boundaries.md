...
Rules for deciding when to split a module, where to place boundaries between
components, and how to assign responsibility in the codebase. Stack-neutral —
...
**Ownership rule** (`harness/data/ownership.yaml`): each zone has a clear owner —
check before creating a new file in a zone owned by someone else.
...
**Name-honesty check:** read the name → guess the content → open the file → compare.
If the reality is narrower than the name → rename or narrow the scope.

Over-description is prohibited: for example, calling `fs_guard` a tool that "blocks
all write operations" instead of "script-path containment helper" is incorrect
(harness-contract.md wording clause).
