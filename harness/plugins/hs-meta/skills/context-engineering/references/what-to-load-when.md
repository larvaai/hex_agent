...
Load when unsure which files to bring into context. Apply progressive disclosure:
metadata -> SKILL.md body -> references/ — in layers, not broadcast everything at once.
...
| Research | hs-research:research -> report path | Raw sources (keep the summarized report) |
| Pack module for LLM | hs-research:repomix -> XML/MD file | Individual files loaded repeatedly |
...
- Output has many `[UNVERIFIED]` tags -> more sources needed
