...
Load when estimating whether a task fits within the context window, or when
weighing single-agent vs multi-agent.
...
**Thumb rule**: 1 Python file ~200 lines ~= 1,500 tokens.
1 Markdown file ~100 lines ~= 800 tokens.
...
| Task 100-160K, subtasks can be cleanly separated | Consider delegating 1-2 subagents |
| Task > 160K or parallel | Multi-agent (~15x token cost) |
| Overlapping files between subtasks | Serialize even if cost is higher |

Accept 15x token cost for multi-agent only when isolation is truly necessary —
not to "look parallel."
...
| hs-research:repomix pack module | More compact than separate file loads | Module > 5 files |
