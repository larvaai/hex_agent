...
For advanced infrastructure/troubleshooting, activate `/hs-devops:devops` skill.
...
Create/update `docs/deployment.md` after the first successful deploy and handle deploy errors (auto-fix → escalate to `/hs-devops:devops`) — detail in `references/post-deploy-and-troubleshooting.md`.
