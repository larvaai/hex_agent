...
Use `hs-devops:agent-browser` for normal browser automation and testing when a fresh or tool-managed browser is acceptable. Use `hs-devops:chrome-profile` only when the task needs the user's actual Chrome profile state.
...
|   +-- YES --> hs-devops:chrome-profile
|   +-- NO --> Continue
|
+-- Browser/app testing, screenshots, forms, scraping, exploratory QA?
|   +-- YES --> hs-devops:agent-browser
|   +-- NO --> Continue
|
+-- Repeatable CI/e2e test suite?
|   +-- YES --> hs-devops:web-testing or project-native Playwright/Vitest/Cypress
|   +-- NO --> Continue
|
...
|   +-- YES --> hs-devops:agent-browser
+-- Otherwise --> hs-devops:web-testing
```
