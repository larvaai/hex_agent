...
Choose the diagram type based on content, then use the corresponding template.
All output includes an ASCII fallback (except `--ascii`, which is ASCII-only).
...
- Verify Mermaid syntax by re-reading the block — common mistakes:
  arrow passing through a box, overlapping labels, duplicate node ids.
...
- Durable architecture diagram -> update `docs/` via `hs-mem:docs`.
