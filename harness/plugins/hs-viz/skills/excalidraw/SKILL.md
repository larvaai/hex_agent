...
license: MIT
keywords: [excalidraw, generate, editable, json, files, canvas]
when_to_use: "Choose when the diagram still needs further editing or an editable source is required."
...
# hs-viz:excalidraw — diagram with reasoning

Generate `.excalidraw` JSON files directly on disk. A good diagram **reasons
visually** — structure communicates the concept on its own, not just labeled boxes.

Runtime path: **file-based** (JSON → `.excalidraw`). Rendering to PNG via
Playwright is optional (see `references/file-workflow.md`). MCP canvas is an
external enhancement, not a dependency.
...
Quick rules: same role = same color; max 3-4 fill colors per diagram;
stroke always darker than fill.
...
The descriptions of excalidraw and `hs-viz:tech-graph` overlap ("draw architecture / data flow"), so
anchor on the **target artifact**, not the subject:
...
- **`hs-viz:tech-graph`** — **SVG/PNG publish-grade** one-shot (8 styles, via `rsvg-convert`); static image embedded in docs/slides.
- **`hs-viz:mermaidjs`** — **inline markdown** diagram, renders natively on GitHub/GitLab.
- **`hs-viz:preview`** — **explains** a change or architecture; does not deliver a diagram file.

SVG layout rules (spacing, arrow routing, label placement, z-index, anti-patterns): see
`hs-viz:tech-graph` — apply when reviewing exported SVG from Excalidraw.

Need to understand the codebase before diagramming: `hs:understand` or `hs-research:repomix`.
