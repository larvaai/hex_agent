...
Analyze any codebase and generate an architecture diagram automatically.
No description needed — read the code, determine what to draw.
...
1. **Read root files**: `package.json`, `pyproject.toml`, `go.mod`, `Cargo.toml`,
   `pom.xml`, `Gemfile`, `Makefile`, `Dockerfile`, `docker-compose.yml`, `*.tf`
...
Use file-based mode (generate `.excalidraw` file).
Follow the sizing rules and color palette from SKILL.md.
