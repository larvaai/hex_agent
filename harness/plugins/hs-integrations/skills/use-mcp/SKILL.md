...
# Read model from $GEMINI_MODEL env (default: gemini-3-flash-preview).
...
```bash
cd ${CLAUDE_PLUGIN_ROOT}/skills/use-mcp/scripts && npm install      # one-time
...
- **Chrome DevTools MCP is profile-blind.** Before using Chrome DevTools MCP navigation, decide whether the task needs a specific real Chrome profile. If not, use the MCP tools normally for generic browser inspection. If it does, invoke `hs-devops:chrome-profile` first. Use `chrome-profile open --json <key> <url>`, select the page whose URL contains the returned `bind_selector`, then use Chrome DevTools MCP only to inspect or operate on that selected page. Do not use raw `new_page` or `navigate_page` for profile-scoped work.
