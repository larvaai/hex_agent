...
An orchestrator is a skill that wraps multiple `hs:*` meta-skills into a fixed workflow
(hs:cook, hs-research:discover, hs:triage, hs:understand). It differs from a leaf skill (does one thing).
...
3. **Chain steps** numbered: each step CALLS a component skill BY NAME
   (`hs-research:research`, `hs:fix`...), <=3 lines per step; details -> drawer.
...
5. **Boundaries**: what NOT to do (e.g. do not edit code directly if hs:fix has been delegated), when
   to escalate, out-of-scope -> `BACKLOG.md`.
6. **References table**.

## Backing-or-cut for orchestrators

Every chain step must point to a component skill that EXISTS (check `load_catalog()['owned']`).
Every gate claim must point to a real gate/schema. No phantoms: if a "gate" is intended but no
suitable artifact/gate exists -> either reuse what is available, or downgrade to a soft handoff.
...
- Isolation nudge (if the handoff carries heavy context) -> author a hook via `hs-create:harness-creator`.
