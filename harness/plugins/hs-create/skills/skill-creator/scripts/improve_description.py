...
Please respond with only the new description text in <new_description> tags, nothing else."""
    ...
    # If over 1024 chars, ask the model to shorten it (claude -p is single-shot,
    ...
    if len(description) > 1024:
        shorten_prompt = (
            ...
            + text
            + f"\n\nThat description is {len(description)} characters, which exceeds the hard "
            "1024 character limit. Rewrite it to be under 1024 characters while preserving the "
        ...
        if shortened:
            ...
            if len(shortened) > 1024:
                print(f"Warning: shortened description is still {len(shortened)} chars "
                      "(>1024); applying as-is", file=sys.stderr)
            transcript["rewrite_still_over_limit"] = len(shortened) > 1024
