...
- Support `response_format`: `markdown` (default, human-readable) and `json`
  (machine-readable)
- Markdown: timestamps as "2024-01-15 10:30 UTC", show name + ID in parentheses,
  drop excess metadata.
...
**Error exposure**: do not leak internal error details to the client — log server-side,
return a useful message without revealing the stack trace.
...
Tool errors must be in the **result object**, not a protocol-level error — so
the LLM can see and handle them:
