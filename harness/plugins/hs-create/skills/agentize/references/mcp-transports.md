...
An MCP server must support **stdio**, **SSE**, and **Streamable HTTP**. One `Server`
instance, three transport adapters.
...
Reject early with `401` + MCP-level error on the first request. Do not leak
whether a token exists. Rate-limit by token.
...
Do not expose metrics without auth. Prometheus if needed: mount on a separate
internal port.
