...
Used after scout + analyze, before scaffolding. Goal: pressure-test assumptions,
surface constraints, and cut scope.

## Core questions (always ask)

1. **Why agentize this?** What is unlocked when an AI agent (not a human) calls
   these operations? If the answer is "it'd be cool" — stop.
2. **Who is the primary consumer?** AI agent, human via CLI, or both? This
   determines output shape, error messages, and verbosity.
3. **What is v1?** Name 3-5 capabilities shipping in week 1. Everything else is v2.
4. **Read vs. write split.** Which capabilities are read-only? Which mutate?
   Which are destructive? (Affects auth, confirmation, and MCP tool design.)
5. **Where do values come from today?** Env? Vault? Hardcoded? This pins the
   defaults in the resolution chain.
6. **Deploy target.** Local-only (stdio + CLI), remote (Cloudflare), self-host
   (Docker), or all? Cost and ops implications differ.
