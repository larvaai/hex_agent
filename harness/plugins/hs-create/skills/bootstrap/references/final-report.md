...
4. **Suggested next steps**
   ...
   - `hs-mem:docs` if documentation needs expanding
   - `hs-flow:project-management` to track progress

## Commit / push

Ask the user: do they want to commit and push?
- Yes -> `git-manager` agent commits (and pushes if requested).
...
After completion (whether or not a commit was made), call `hs-mem:journal` to record the session.
