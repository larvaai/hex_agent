...
Analyzers must not exit 2 — on error, return JSON with an `"error"` field and exit 0.
The caller (gate or skill) decides whether to block based on the result.
...
The block reason must be actionable: the reader knows what to do to unblock.
Do not return a raw exception message or a numeric code.
...
`TestStoreWriteDiscipline` in `test_bug_class_invariants.py` catches any
`open(..., "w"/"a")` pointing into `state/` from code outside the whitelist writers.

## fs_guard — script-path containment helper

`harness/scripts/check_fence.py` is the **script-path containment helper** — it
limits harness scripts to writing only within their declared zone. This is a
boundary for harness scripts, not for the LLM Write tool. Do not describe
fs_guard as anything else — `TestWording` catches incorrect descriptions.

## Additional rules

- Filename: `snake_case.py` (Python convention).
- Every script requires TDD: write a test in `harness/tests/` before implementing.
- Scripts must not import from `harness/plugins/hs/skills/` — coupling between
  runtime scripts and skill prose is not allowed.
- Use `harness_paths.root()` to resolve absolute paths instead of `os.getcwd()`.
- Heavy imports (PyYAML, networkx) should be lazy-loaded inside functions — keeps
  import time fast and does not block when a dependency is missing.
