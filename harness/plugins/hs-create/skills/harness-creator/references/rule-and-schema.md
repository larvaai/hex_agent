...
A rule is an on-demand contract: agents and skills load it when the task needs it, not
always in context. Create a rule when:

- An invariant or contract needs to be defined for use by multiple skills or hooks.
- Detail should be separated from SKILL.md (thin-core) — rules replace references
  drawers for content shared across the harness (not shared = references drawer).
- Do not create a rule for content relevant to only one skill -> place it in that
...
- No CI test is required for a rule (prose), but if a rule describes an invariant
  then that invariant must have a test in `harness/tests/`.
...
A schema describes the contract of a **machine-written artifact** that a gate reads.
Only create a schema when:

- A real gate or script will read this artifact to make a block/pass decision.
- The artifact needs to be validated before the gate accepts it (no schema needed if
  the artifact is only a passive log).
...
**Do not mix**: YAML for config/policy, JSONL/JSON for machine-written data.
Do not use YAML for gate artifacts — gates require deterministically parseable JSON.
...
- Store writes: **append-only, no read-modify-write**. Only via
  `trace_log.append_event` or `telemetry_paths.append_event` — do not open `state/`
  files directly from code outside the shared writers.
