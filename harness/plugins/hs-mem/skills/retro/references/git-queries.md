# Git Queries — hs-mem:retro

Bash commands to collect raw data. Run in order. Record `0` or `N/A`
if a command returns nothing — never invent numbers.
...
Re-run all queries above using `$PREV_SINCE` / `$PREV_UNTIL`.
Store results in corresponding `PREV_*` variables to compute deltas.
