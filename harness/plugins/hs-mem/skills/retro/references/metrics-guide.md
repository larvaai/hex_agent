# Metrics Guide — hs-mem:retro

Definitions, formulas, and interpretation thresholds for each metric. Include
the formula in the report so readers can verify the figures.
...
**Key threshold:** a file appearing in > 30% of commits likely has high coupling —
consider refactoring. Top 3 hotspots per period → include in action items if recurring.

---

## Commit Distribution

**Measures:** proportional distribution of conventional commit types (`feat`, `fix`,
...
`+` on commit frequency, active ratio, test ratio = improvement.
`+` on churn rate = degradation — note the direction clearly in the report.
