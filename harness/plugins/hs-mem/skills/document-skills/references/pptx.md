...
A `.pptx` file is a ZIP containing XML. Primary library: **python-pptx** (read/create/edit).
When raw XML is needed (animations, comments, parts not covered by python-pptx) -> unzip with
the standard `zipfile` library, edit with `lxml`, zip back (same as docx.md). No external script needed.
...
- Cross-platform safe fonts: Arial, Helvetica, Calibri, Times New Roman,
  Georgia, Verdana, Tahoma.
- Slides with charts/tables: use **two-column** layout (40/60) or full-slide; do NOT
  stack a chart below text in a single narrow column.
...
Validate by rendering to images (section 5) and check: clipped text, overlaps,
poor contrast -> fix code -> regenerate.
...
Edit run-by-run to preserve formatting; avoid assigning `shape.text_frame.text = ...` because
it erases all existing runs and formatting. For XML parts not covered by python-pptx (animations,
transitions) -> use raw XML from section 1.
...
- Use `RGBColor(0xFF, 0x00, 0x00)` for RGB colors; theme colors via
  `MSO_THEME_COLOR` (`from pptx.enum.dml import MSO_THEME_COLOR`).
- To duplicate or reorder slides, operate on `prs.slides._sldIdLst` (the list of
  `<p:sldId>` elements in `presentation.xml`) via lxml — move/copy elements,
  then save.
