...
`.docx` is a ZIP file. Unzip with the standard `zipfile` library, edit the XML with `lxml`,
then zip back — no external script required:
...
When tracked changes or XML features are needed that python-docx does not cover, use raw OOXML
(unzip -> edit `word/document.xml` with lxml -> zip back, see section 1).
XML editing rule — **only mark the parts that actually changed**:
```python
...
3. **Unpack + read OOXML doc** (zipfile, no external script needed):
   ...
   with zipfile.ZipFile("file.docx") as z:
       z.extractall("unpacked/")
   ```
   Tracked changes need a consistent RSID (8 hex digits, e.g. `00AB12CD`);
   generate one value and reuse it for all `<w:ins>`/`<w:del>` in the batch.
