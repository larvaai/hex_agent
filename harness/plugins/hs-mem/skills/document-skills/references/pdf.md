...
No fields to fill -> draw text at the correct coordinates then merge it over the original page.
PDF uses a bottom-left origin coordinate system; unit is points (72pt = 1 inch).
...
1. **Determine coordinates**: render the page to an image to locate the positions to fill.
   ...
   ```
   Measure y from the top of the image then convert to bottom-left origin:
   `y_pdf = page_height_pt - y_top_pt`.
...
4. **Verify**: render `output.pdf` back to an image, check text lands in the correct blank space
   and does not overlap labels; adjust coordinates in step 2 and repeat if needed.
