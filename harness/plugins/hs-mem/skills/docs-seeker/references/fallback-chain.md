...
4. WebSearch
   ...
   ↓ not found
5. Repo analysis — hs-research:repomix
   (see section below)
```
...
2. WebSearch "[library] llms.txt"
   ↓ not found
3. hs-research:repomix on GitHub repo
   ↓ no repo
4. hs-research:research with multiple web sources
   (docs / blog / README / tutorials)
```

## Repo analysis via hs-research:repomix

Use when llms.txt is completely absent:

1. WebSearch `"[library] github repository"` — confirm the official repo is active.
2. Call `hs-research:repomix` with the repo URL — returns a single-source XML file.
