...
Use when the user asks about a specific feature or component. 10x faster than
the general pattern and uses fewer tokens.
...
- Specific version: WebSearch `"[library] v[version] llms.txt"` or try
  paths `/v{version}/llms.txt`, `/docs/v{version}/llms.txt`.
