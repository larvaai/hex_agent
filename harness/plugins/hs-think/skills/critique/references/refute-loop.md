# hs-think:critique — refute loop (on-demand, `--loop` only)

Load this ONLY when the user passes `--loop`. A normal critique runs once: lenses -> consolidate ->
report. The loop adds a defense pass so a contested verdict is tested before it stands. Pattern mirrors
`harness/plugins/hs-think/skills/predict/references/chain-modes.md` (`--chain reason`).

## When to loop

Use `--loop` when the consolidated verdict is contested — a blocker that the artifact's author would
plausibly rebut, or a `suspected` finding doing load-bearing work. Skip it when the verdict is already
clean (PASS, nothing to defend) or when every blocker is `proven` and unarguable (defending a proven
data-loss path wastes cycles — fix it).
...
2. Refute
   ...
   - The refutation is held to the same Evidence Filter: a rebuttal needs its own anchor
     (file:line / repro / input) or it is itself [UNVERIFIED] and does not overturn the finding.
...
3. Re-consolidate
   ...
   - A finding whose rebuttal is only [UNVERIFIED] stands.
...
Tone stays neutral: the refute pass argues with the finding's evidence, never with the lens or the
author. A rebuttal that cannot anchor does not win.
