...
Load when in the Analysis phase and the `brainstormer` agent needs a structure for
exploring multiple directions. Do not read in --critique mode.
...
In every brainstorm, explicitly identify the option with the least complexity that still
meets the requirements. Name it "MVS" (minimum viable solution) and place it first on the
list. Default to the MVS unless there is specific evidence it is insufficient.

## Scope decomposition

A request describing ≥3 independent concerns (e.g., "build platform with chat,
billing, analytics") → decompose first:
...
- **Analogy mapping**: how is a similar problem solved in another domain?
  Map the analogy onto the current domain.
- **Failure-first**: start from "what breaks fastest" → work backward to a
  design that avoids that failure.
