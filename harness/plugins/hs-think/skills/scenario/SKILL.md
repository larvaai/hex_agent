---
name: hs-think:scenario
description: Decompose a feature/code path across 12 dimensions to generate edge cases, risks, and test targets before implementation. Use for QA planning, risk discovery, and coverage audits.
category: think
license: AGPL-3.0
keywords: [scenario, decompose, feature, code, path, across]
when_to_use: "Use for QA planning, risk discovery, and coverage audits."
...
# hs-think:scenario — Edge case & scenario explorer

Decomposes any feature or code path across 12 dimensions to surface edge cases, risks,
and test targets **before implementation**.
...
Not every dimension applies — **filter first**, generate scenarios only for applicable
dimensions, and explicitly note which are skipped.
...
2. Filter the 12 dimensions — note skipped dimensions + reason
...
Each round: pick a dimension -> generate 1 situation -> classify (New/Variant/Duplicate) ->
log to `scenario-results.tsv` -> check halt condition.
Print progress summary every 5 rounds.
...
| Debate top risks in depth | `hs-think:predict` | Feed Critical scenarios as the change proposal |
| Research unclear risks further | `hs-research:research` | Use when evidence is needed for an unclear risk |

## Examples

```bash
# One-shot (default)
/hs-think:scenario src/api/payment.ts
/hs-think:scenario "User registration with OAuth providers"

# Bounded — exactly 25 rounds
/hs-think:scenario src/api/payment.ts --iterations 25

# Saturation — stops automatically when coverage is exhausted
/hs-think:scenario "Add multi-tenancy to the database layer" --saturation

# Security domain hint
/hs-think:scenario src/middleware/auth.ts --saturation --domain security
```
