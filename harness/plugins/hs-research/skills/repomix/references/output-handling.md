# output-handling — token management and artifact handling for hs-research:repomix

Load this file when you need to estimate tokens, choose a format, or handle a
large digest before loading it into an LLM.
...
> Use `hs-meta:context-engineering` to check the remaining budget before loading a digest.
...
Load them sequentially; use `hs-meta:context-engineering` after each load to check
...
scope determined?
  ...
  └─ yes -> estimate tokens (--token-count-tree)
             ...
             └─ over budget   -> narrow scope or split
                                 └─ still large -> hs-meta:context-engineering
```
