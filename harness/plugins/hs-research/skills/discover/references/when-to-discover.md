# When to discover — when to use hs-research:discover

Load this drawer in step 1 of hs-research:discover when the problem may not be clear enough,
or when the user asks "do I need to discover?"

## Signs that hs-research:discover SHOULD run
...
| Prior art or existing codebase solutions are unknown | hs-research:research needed |
...
## Signs to SKIP hs-research:discover (go straight to hs:plan)
...
Trigger: after diverge there are still not >=2 distinct directions, or
after critique every option is Rejected with no alternative.

Steps to handle:
1. Call `hs-think:problem-solving` with a specific description of where the block is
...
The brief is a map for hs:plan to create a plan — it is not a plan itself.
Signs the brief is creeping into plan territory:
...
When this happens -> stop, move the content into "Explicitly OUT of scope" or write it
to `BACKLOG.md`, do not pack it into the brief.
