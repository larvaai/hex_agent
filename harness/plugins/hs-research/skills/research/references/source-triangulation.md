...
Run in step 2, before synthesis. Goal: do not draw conclusions from a single source;
every important claim must have at least 2 independent sources.
...
3. **Production case study** — article from a company using it in practice (Stripe, Shopify,
   Netflix eng blog...)
...
To confirm whether a repo file or path EXISTS, query the authenticated API, not a
web search. Use `gh api repos/{owner}/{repo}/contents/{path}` (or the project's API
with a token). An unauthenticated code search returns a "sign in to search" page,
`0 results`, or HTTP 429 — none of which mean the file is absent. Absence of a
search result is not evidence of absence. Never conclude "the feature/file does
not exist" from an empty or rate-limited unauthenticated search.
...
If fewer than 2 independent sources exist for an important claim -> tag `[UNVERIFIED]`,
add an open question at the end of the report.
