...
1. **`hs-uiux:ui-ux-pro-max`** - Design intelligence database (ALWAYS FIRST)
2. **`hs-uiux:frontend-design`** - Screenshot analysis and design replication
3. **`hs-uiux:web-design-guidelines`** - Web design best practices
4. **`hs-stack:react-best-practices`** - React best practices
5. **`hs-stack:web-frameworks`** - Web frameworks (Next.js / Remix) and Turborepo
6. **`hs-uiux:ui-styling`** - shadcn/ui, Tailwind CSS components

**Before any design work**, run `hs-uiux:ui-ux-pro-max` searches:
```bash
...
**Gemini Image Generation (`hs-ai:ai-multimodal` skills)**:
...
**Gemini Vision (`hs-ai:ai-multimodal` skills)**:
...
- Analyze and optimize generated assets from `hs-ai:ai-multimodal` skills and `imagemagick` skills

**Screenshot Analysis with `hs-devops:agent-browser` and `hs-ai:ai-multimodal` skills**:
- Capture screenshots of current UI
- Analyze and optimize existing interfaces
- Compare implementations with provided designs

**Figma Tools**: use Figma MCP if available, otherwise use `hs-ai:ai-multimodal` skills
- Access and manipulate Figma designs
- Export assets and design specifications

**Google Image Search**: use `WebSearch` tool and `hs-devops:agent-browser` to capture screenshots
...
1. **Research Phase**:
   ...
   - Generate a comprehensive design plan using `hs:plan` skills
...
2. **Design Phase**:
   ...
   - Generate/modify real assets with hs-ai:ai-multimodal skill for images and ImageMagick for editing
   - Generate vector assets as SVG files
   - Always review, analyze and double check generated assets with hs-ai:ai-multimodal skill.
...
4. **Validation Phase**:
   - Use `hs-devops:agent-browser` to capture screenshots and compare
   - Use `hs-ai:ai-multimodal` skills to analyze design quality
   - Use `imagemagick` skills or `hs-ai:ai-multimodal` skills to edit generated assets
   - Conduct accessibility audits
   - Gather feedback and iterate

5. **Documentation Phase**:
   - Update `./docs/design-guidelines.md` with new patterns
   - Create detailed reports using `hs:plan` skills
