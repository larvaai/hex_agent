...
**All workflows**: Activate `hs-uiux:ui-ux-pro-max` FIRST for design intelligence.

**Precedence:** When anti-slop rules conflict with `hs-uiux:ui-ux-pro-max` recommendations (e.g., Inter font, AI Purple palette, Lucide-only icons), substitute alternatives from `./references/anti-slop-rules.md` unless the user explicitly requested the conflicting choice.
...
1. **Analyze** with `hs-ai:ai-multimodal` — extract colors, fonts, spacing, effects
...
1. **Brainstorm a compact design plan** — Subject (concrete subject/audience/screen-job); Color (4-6 named hex tokens with roles); Type (≥2 roles, display + body, + data face when needed); Layout (1-2 concepts as prose/ASCII wireframes); Signature element; Motion (one role, or an explicit decision to stay still); Copy voice.
...
- **Tone**: Pick an extreme (brutally minimal, maximalist, retro-futuristic, organic, luxury, playful, editorial, brutalist, art-deco, pastel, industrial...). Design one true to the direction.
