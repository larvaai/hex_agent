...
Complete workflow for generating design-aligned visual assets using `hs-ai:ai-multimodal` skill.
...
Use `hs-ai:ai-multimodal` skill's image generation capabilities:
...
Use `hs-ai:ai-multimodal` to analyze the generated asset (see `visual-analysis.md` for complete workflow):
...
4. Consider using `hs-ai:media-processing` skill for post-generation adjustments
