...
Entry point for using the `hs-ai:ai-multimodal` skill to generate and analyze visual assets that align with frontend design thinking and aesthetic guidelines.

## When to Use

Use `hs-ai:ai-multimodal` in frontend design when you need to:
...
Use `aesthetic` for overall design system guidance and quality evaluation framework. Then use `hs-uiux:frontend-design` with `hs-ai:ai-multimodal` for asset generation and analysis that follows those guidelines.

### With `hs-devops:agent-browser`
Use `hs-devops:agent-browser` to capture screenshots from inspiration websites for design extraction. Capture at actual viewport size, not full-page scrolls.
...
### With `hs-ai:media-processing` Skill
