...
The preference helper at `${CLAUDE_PLUGIN_ROOT}/skills/show-off/scripts/preferences.js`:

```bash
# Print resolved user preferences as JSON
node ${CLAUDE_PLUGIN_ROOT}/skills/show-off/scripts/preferences.js get

# Persist workflow opt-outs
node ${CLAUDE_PLUGIN_ROOT}/skills/show-off/scripts/preferences.js set --no-screenshots --no-publish --languages en

# Re-enable defaults
node ${CLAUDE_PLUGIN_ROOT}/skills/show-off/scripts/preferences.js reset
```
...
Preferences persist at `~/.claude/show-off/preferences.json` by default; `SHOW_OFF_PREFS_PATH`
overrides the path for tests or one-off advanced use.

## Capture Script

The parallel capture script at `${CLAUDE_PLUGIN_ROOT}/skills/show-off/scripts/capture-sections.js`:

```bash
# Capture all sections in parallel across multiple ratios
node ${CLAUDE_PLUGIN_ROOT}/skills/show-off/scripts/capture-sections.js \
...
The same chain runs again after `scrollIntoView()` per section, so reveal-on-scroll animations
capture correctly.

## `rws` Publish Fallback

If `publishing=true` and the local capture script fails (puppeteer missing, headless Chrome
unavailable, sandbox error, non-zero exit) AND `rws` is on PATH AND `$RWEB_API_KEY` is set, fall
back to the ReviewWeb screenshot API. The HTML must be publicly reachable (publish via `agentwiki`
first, then use the public URL + `#section-id` anchors).
...
- Skip `rws` if the HTML is only reachable via `file://` and cannot be published yet — surface the
  local script error and stop.
- Never pass `$RWEB_API_KEY` on the command line; `rws` reads it from the env automatically.
- On `rws` exit code 2 (auth error) or missing `$RWEB_API_KEY`, stop and report — do not silently
  skip capture.
