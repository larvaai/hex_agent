...
Each worktree has its own working directory but shares the repo's `.git/`.
The same branch cannot be checked out in two worktrees simultaneously —
git reports "already checked out".

To commit/push from inside the worktree, use `hs:git` (secret scan, conventional commit,
push gate via `harness/install/git-pre-push-hook.sh`).
...
After the directory is removed, git cleans up the metadata automatically. If deleted manually
(rm -rf) without going through `git worktree remove`, run `git worktree prune` to clean up.
...
Prune only deletes metadata in `.git/worktrees/` — does not touch the external filesystem.
