...
When stopping: record 1 event in `harness/hooks/trace_log.py`
(`event: loop_end`, `status: complete|stuck|interrupted|goal_met`).
...
Never relax the Guard or modify the Guard file to pass — Guard failure means the
change is wrong, not that the Guard is wrong.
...
Reason: `git log --grep="loop(iter-"` depends on intact history. Reset destroys
try-discard data needed for pattern analysis.
...
2. **No-new-work**: Scope is fully explored (no files/techniques left untried) or
   10 consecutive DISCARDs after rotating strategy.
...
- Further unattended run → suggest `hs-flow:afk` for Docker sandbox + reviewer-stage.
