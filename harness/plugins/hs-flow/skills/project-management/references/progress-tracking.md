...
3. **Scan phase files**: count `[x]` (done) and `[ ]` (remaining) in each phase file.
4. **Reconcile**: ensure completed tasks have been mapped and backfilled into phase files —
   backfill earlier phases BEFORE counting later phases.
...
If no CLI plan tool is available — edit `plan.md` directly:
change only the Status cell in the table; preserve the rest of the structure.
...
Each `phase-XX-*.md` is tracked with checkboxes:
- `[ ]` = pending
- `[x]` = completed
