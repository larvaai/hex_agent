...
| Researcher | `researcher` | Investigate one angle; save report; notify lead when done |
| Developer | `developer` | Implement in a dedicated worktree; owns the assigned glob patterns |
| Tester | `tester` | Run full suite after dev; owns test files only; reads but does not edit implementation |
| Reviewer | `code-reviewer` | Review one focus area; findings must have evidence; does not edit code |
| Debugger | `debugger` | Verify one hypothesis; actively challenges other hypotheses |
