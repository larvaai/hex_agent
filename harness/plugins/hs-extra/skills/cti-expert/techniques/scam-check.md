...
**Method:** WebSearch for `site:virustotal.com "[DOMAIN]"` or fetch via `agent-browser`; use `hs-devops:chrome-profile` only for real logged-in Chrome state:
```
...
4. agent-browser - for JavaScript-heavy pages; hs-devops:chrome-profile only when real user Chrome cookies are required (VirusTotal, Scamadviser)
