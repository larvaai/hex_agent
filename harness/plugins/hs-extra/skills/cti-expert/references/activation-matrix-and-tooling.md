...
1. Check `agent-browser` availability first; use `hs-devops:chrome-profile` only if real user Chrome cookies are required
2. Use `agent-browser` for generic screenshot evidence and interactive UI; use `hs-devops:chrome-profile` only when real Chrome profile state is required
