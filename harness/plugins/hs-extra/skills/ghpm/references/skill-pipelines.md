...
```text
ghpm -> hs:scout -> hs-research:research -> hs:plan
```
...
3. `hs-research:research`: research unstable APIs or external dependencies; attach report link.
...
```text
ghpm -> hs-extra:watzup -> hs-flow:project-management
```

Use when another human or session needs continuation:

- `hs-extra:watzup`: summarize branch/worktree/unfinished work.
- `hs-flow:project-management`: reconcile local plan status if plans exist.
