...
```bash
node ${CLAUDE_PLUGIN_ROOT}/skills/watzup/scripts/watzup-scan.cjs --json
```

Use `--fetch` only when the user asks to refresh remotes before the report:

```bash
node ${CLAUDE_PLUGIN_ROOT}/skills/watzup/scripts/watzup-scan.cjs --json --fetch
```
