...
3. ./.claude/.env
4. <plugin>/skills/.env (shared across skills)
...
def find_api_key(skill_dir: Optional[Path] = None) -> Optional[str]:
    ...
    # Step 4: Check the shared skills .env (<plugin>/skills/.env)
    shared_skills_env = Path(__file__).resolve().parent.parent / '.env'
    if shared_skills_env.exists():
        api_key = load_env_file(shared_skills_env)
        if api_key:
            print(f"✓ Using API key from {shared_skills_env}", file=sys.stderr)
            return api_key
...
def get_api_key_or_exit(skill_dir: Optional[Path] = None) -> str:
    ...
    if not api_key:
        ...
        _shared_skills_env = Path(__file__).resolve().parent.parent / '.env'
        print("\n4️⃣  Shared skills .env file (shared across skills):", file=sys.stderr)
        print(f"   echo 'GEMINI_API_KEY=your-api-key' > {_shared_skills_env}", file=sys.stderr)
