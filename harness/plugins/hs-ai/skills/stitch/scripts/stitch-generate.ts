 ...
 *   5. "harness-default"     last resort fallback
 */
...
const QUOTA_DIR = path.join(os.homedir(), ".sdlc-harness");
...
if (directProjectId) {
  ...
  projectId = detected || "harness-default";
  resolvedProjectName = projectId;
  isNameBasedProject = true;
  if (detected) {
...
async function main() {
  try {
    ...
    if (remaining < creditsNeeded) {
      ...
      console.error("[i] Use hs-uiux:ui-ux-pro-max as fallback, or wait until midnight UTC.");
      process.exit(2);
    }
    ...
    if (err.code === "RATE_LIMITED") {
      ...
      q.count = q.limit;
      saveQuota(q);
      console.error("[X] Daily quota exceeded (local tracker synced). Try tomorrow or use hs-uiux:ui-ux-pro-max.");
