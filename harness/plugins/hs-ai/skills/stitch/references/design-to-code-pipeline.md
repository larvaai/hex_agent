...
# 4. Activate hs-uiux:frontend-design to implement React components from the spec
```
...
|-------------|-------|-------------|
| React/Vue/Svelte components | `hs-uiux:frontend-design` | Converts Tailwind HTML to framework components |
| Full page with style guide | `hs-uiux:ui-ux-pro-max` | Builds complete layouts with design system |
| Design token extraction | `hs-uiux:ui-styling` | Extracts shadcn/Tailwind tokens from DESIGN.md |

## Handoff Protocol

1. `hs-ai:stitch` exports artifacts to `./stitch-exports/` (or plan directory)
...
# -> Skip Stitch, use hs-uiux:ui-ux-pro-max for text-based design instead
