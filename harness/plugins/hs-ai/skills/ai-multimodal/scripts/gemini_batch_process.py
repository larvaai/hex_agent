...
def find_api_key() -> Optional[str]:
    ...
    Uses resolve_env.py for consistent resolution across all skills.
    Falls back to local resolution if centralized resolver not available.

    Priority order (highest to lowest):
    1. process.env (runtime environment variables)
    2. <project>/<skill>/.env (skill-specific)
    3. <project>/<skills>/.env (shared skills)
    4. <project>/.env (project global)
    5. ~/<skill>/.env (user skill-specific)
    6. ~/<skills>/.env (user shared)
    7. ~/.env (user global)
    """
...
def batch_process(
    ...
    if requires_google_sdk:
        ...
        if not api_key:
            ...
            print("  - Show full hierarchy: python resolve_env.py --show-hierarchy --skill ai-multimodal -v")
