...
def load_env_files():
    ...
    2. <skill>/.env (skill-specific config)
    3. <skills>/.env (shared skills config)
    4. .env (Claude global config)
    """
    if not load_dotenv:
        return
    ...
    skill_dir = script_dir.parent  # the skill directory
    skills_dir = skill_dir.parent   # the skills directory
