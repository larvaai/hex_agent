...
Nudge the user to isolate context between discovery and planning. hs-research:discover
...
warns when hs-research:discover ran earlier in the SAME session_id — a signal the
...
_DISCOVER_SKILLS = {"hs-research:discover", "discover"}
...
def _discover_in_session(session: str) -> bool:
    """Best-effort: was hs-research:discover logged for this session_id? Reads the
...
def core(data: dict) -> None:
    """Emit the advisory iff the incoming skill is hs:plan AND hs-research:discover ran in
    the same session (discovery context likely not cleared). stderr only — never
    blocks."""
    ...
    if not _discover_in_session(session):
        return
    sys.stderr.write(
        "[nudge] discover_isolation: hs-research:discover already ran in this session — "
...
def main() -> int:
    ...
    # detector fail-open, then ALWAYS continue. A disabled hook is fully inert.
