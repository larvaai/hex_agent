...
moving?". If so it surfaces a ONE-line advisory pointing at /hs-mem:remember and
records one observation in the audit trace — then ALWAYS allows. It never blocks,
never writes memory or the ledger; the human (or /hs-mem:remember) does the writing.
...
def _advisory_text(signal: Dict[str, Any]) -> str:
    ...
    return ("decision-capture: %d decision-shaped change(s) shipped without a "
            "ledger entry: %s. Run /hs-mem:remember to draft a DEC/memory, or record it "
...
def handle_stop(payload: Dict[str, Any], project_dir: Optional[str] = None) -> int:
    ...
    if signal:
        _record_observation(actor, session_id, signal)
        sys.stderr.write("[advisory] %s\n" % _advisory_text(signal))
    return ALLOW_EXIT
