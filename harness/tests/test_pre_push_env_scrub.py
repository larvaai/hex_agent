...
class TestScrub:
    def test_blocks_without_artifact_baseline(self, repo):
        proc = _push(repo)
    ...
    def test_protected_branch_push_demands_merge_artifacts(self, repo):
        ...
        assert proc.returncode != 0
        assert "review-decision" in proc.stderr
    ...
    def test_unknown_future_harness_var_is_scrubbed_too(self, repo, tmp_path):
        ...
        assert proc.returncode != 0  # still judged by the repo's own plan
...
class TestDocstringHonesty:
    def test_hook_no_longer_claims_verify_install_sees_removal(self):
        # The active copy at .git/hooks/pre-push is OUTSIDE the manifest;
        # plain removal is NOT visible to a later verify_install run. The
        # comment must state the real mechanism (the --strict copy check),
        # not the old overclaim.
        text = _HOOK_SRC.read_text(encoding="utf-8")
        assert "removal is visible in any later verify_install run" not in text
