...
(hs-research:discover) carries heavy research/debate context that biases planning toward
...
when hs-research:discover ran earlier in the SAME session_id (discovery context not
...
def test_nudges_when_discover_same_session(tmp_path):
    _seed(tmp_path, "hs-research:discover", "S1")
...
def test_silent_when_discover_other_session(tmp_path):
    _seed(tmp_path, "hs-research:discover", "OTHER")
    r = _run(tmp_path, _cfg(tmp_path, _ENABLED), _plan("S1"))
...
def test_disabled_by_default_is_inert(tmp_path):
    _seed(tmp_path, "hs-research:discover", "S1")            # carryover present...
