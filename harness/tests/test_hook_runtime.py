...
class TestGuardPolicyBridge:
    ...
    def test_registered_nudge_off_at_balanced(self, tmp_path, monkeypatch):
        # Nudges ship asleep at the default preset.
        hr = _fresh(monkeypatch, tmp_path, config={"hooks": {}}, preset="balanced")
        assert hr.hook_enabled("cook_isolation_nudge", "nudge") is False

    def test_registered_nudge_warn_at_strict_enables_but_advisory(self, tmp_path, monkeypatch):
        hr = _fresh(monkeypatch, tmp_path, config={"hooks": {}}, preset="strict")
        assert hr.hook_enabled("cook_isolation_nudge", "nudge") is True
        # Class still caps a nudge at advisory: posture cannot escalate it.
        assert hr.hook_mode("cook_isolation_nudge", "nudge") == "advisory"
