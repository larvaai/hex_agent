...
def test_severity_badge_known_levels():
    assert tf.severity_badge("high") == "[!!!]"
    assert tf.severity_badge("medium") == "[!!]"
    assert tf.severity_badge("low") == "[!]"
    assert tf.severity_badge("critical") == "[XXX]"
    assert tf.severity_badge("info") == "[i]"


def test_severity_badge_case_insensitive():
    assert tf.severity_badge("HIGH") == tf.severity_badge("high")


def test_severity_badge_unknown_passes_through_bracketed():
    assert tf.severity_badge("weird") == "[weird]"


# --- json/summary ---

def test_json_output_roundtrips():
    assert json.loads(tf.json_output({"a": 1}, pretty=False)) == {"a": 1}
    assert json.loads(tf.json_output({"a": 1}, pretty=True)) == {"a": 1}


def test_summary_block_renders_title_and_items():
    out = tf.summary_block("Title", {"k1": "v1", "k2": 2})
