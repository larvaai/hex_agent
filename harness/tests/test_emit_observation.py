...
def test_real_vocab_file_loads(tmp_path):
    # the shipped observation-signals.yaml parses and carries the documented signals
    real = _SCRIPTS.parent / "data" / "observation-signals.yaml"
    names = eo.load_vocab(real)
    assert "thin-evidence" in names
    assert "gate-repeat-block" in names
