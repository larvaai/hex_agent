...
_SKILL_DIR = _ROOT / "harness" / "plugins" / "hs-research" / "skills" / "security-scan"
...
class TestComplianceReference:
    ...
    def test_maps_frameworks_to_stride_or_owasp_controls(self):
        body = _read(_COMPLIANCE)
        # each framework row must anchor to a concrete control, not float free
        assert "OWASP" in body or re.search(r"\bA0[1-9]\b|A10", body)
        assert "STRIDE" in body or re.search(r"\b[STRIDE]\b", body)
