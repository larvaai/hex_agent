...
findings_store is hs-flow:compound's belief memory. It departs from the source instinct_store:
...
def test_record_has_actor_and_ts(tmp_path):
    p = _store(tmp_path)
    obs = fs.emit_observation_record("idea needing attribution", "skills", path=p)
    rein = fs.emit_reinforce(obs["id"], path=p)
    for rec in (obs, rein):
        assert rec.get("actor")  # resolve_actor() output, non-empty
        assert fs.parse_iso_ts is not None
        assert fs.parse_iso_ts(rec["ts"]) is not None  # ts is a parseable ISO string
