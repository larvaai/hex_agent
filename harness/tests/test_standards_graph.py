...
def test_depends_on_reserved(tmp_path):
    root = _write_valid_tree(tmp_path)
    graph = standards_graph.build_graph(root)
    r2 = _node(graph, "STD-AUTH-RG1-R2")
    assert r2["depends_on"] == ["STD-AUTH-RG1-R1"]


def test_no_product_only_fields(tmp_path):
    # the standards node must not import the product-only field family
    root = _write_valid_tree(tmp_path)
    graph = standards_graph.build_graph(root)
    banned = {"competitors", "competitive_parity", "risks", "moscow",
              "horizon", "size", "scope", "personas", "brd_goals"}
    for n in graph["nodes"]:
