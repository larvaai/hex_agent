...
class TestNoTokenLeak:
    ...
    def test_provider_source_never_serializes_headers_into_errors(self):
        # Static invariant: no error construction in the provider source
        # interpolates request headers (the only place the token lives).
        src = (_SCRIPTS / "task_store_github.py").read_text(encoding="utf-8")
        for line in src.splitlines():
            if "Error(" in line or "Unavailable(" in line or "RateLimited(" in line:
                assert "header" not in line.lower(), line
                assert "Authorization" not in line, line
