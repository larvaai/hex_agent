...
class TestBlocking:
    def test_push_without_plan_blocks_exit_two_with_three_exits(self, tmp_path):
        proc = _run(tmp_path, "git push")
        assert proc.returncode == 2
        assert "HARNESS_ACTIVE_PLAN" in proc.stderr
        events = [e for e in _trace_events(tmp_path) if e["event"] == "gate_block"]
        assert events and events[0]["actor"] == "user:alice"

    def test_push_with_plan_but_no_artifact_blocks_naming_it(self, tmp_path):
        _mk_plan(tmp_path)
        proc = _run(tmp_path, "git push")
        assert proc.returncode == 2
...
class TestPrStagePlanApproval:
    ...
    def test_pr_without_plan_approval_blocks_exit_two(self, tmp_path):
        self._full_plan(tmp_path)
        proc = _run(tmp_path, "gh pr create --fill")
        assert proc.returncode == 2
        assert "plan-approval" in proc.stderr
