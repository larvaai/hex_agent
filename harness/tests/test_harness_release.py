...
class TestCommittedReleaseFile:
    def test_committed_release_json_valid(self):
        root = Path(__file__).resolve().parents[2]
        rel_path = root / "harness" / "release.json"
        # release.json is the SOURCE version identity, excluded from the manifest
        # and not shipped in the bundle. This test ships under harness/tests/ and
        # runs at deployer sites too, where the file is absent — skip there.
        if not rel_path.is_file():
            pytest.skip("harness/release.json absent (source-only, not shipped)")
        rel = json.loads(rel_path.read_text(encoding="utf-8"))
        for key in ("schema_version", "harness_version", "channel"):
            assert key in rel, f"release.json missing {key}"
