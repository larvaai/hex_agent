...
def test_grammar_single_home(tmp_path):
    # the generator must import the grammar, never re-encode it
    src = (_SCRIPTS / "generate_standards_templates.py").read_text(encoding="utf-8")
    assert "ID_PATTERN_BY_TYPE" in src
    assert "standards_graph" in src
