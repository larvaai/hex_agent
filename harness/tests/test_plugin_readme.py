...
def test_readme_indexes_skill_invoke_names():
    # the hs spine README must list at least its core invoke names
    body = (_PLUGINS / "hs" / "README.md").read_text(encoding="utf-8")
    for inv in ("/hs:plan", "/hs:cook", "/hs:ship"):
        assert inv in body, "hs README missing %s" % inv
...
def test_writes_when_absent_or_marked(tmp_path):
    plug = _make_fake_plugin(tmp_path, "hs-demo", [("hs-demo:alpha", "Do alpha.")])
    assert gpr.write_plugin_readme(plug) is True          # absent -> write
    assert gpr.MARKER in (plug / "README.md").read_text(encoding="utf-8")
    assert gpr.write_plugin_readme(plug) is True          # has marker -> regenerate ok
