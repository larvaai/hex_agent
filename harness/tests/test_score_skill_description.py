...
confusable-pair detection via Jaccard similarity, and dependency cycle detection via DFS.
"""
...
def _skill(name: str, description: str, requires: list[str] | None = None) -> dict:
    s: dict = {"name": name, "description": description}
...
class TestScoreDescription:
    ...
    def test_score_has_correct_skill_name(self):
        result = ssd.score_description("my-skill", "Analyze things. Use when needed, always.")
        assert result.skill_name == "my-skill"

    def test_boundary_score_defaults_to_one(self):
...
class TestDependencyCycles:
    def test_simple_cycle_detected(self):
        # a -> b -> a
        skills = [
            _skill("a", "skill-a description", requires=["b"]),
            _skill("b", "skill-b description", requires=["a"]),
        ]
        errors = ssd.validate_dependency_graph(skills)
        assert errors, "Expected a cycle to be detected"
        assert any("a" in e and "b" in e for e in errors)

    def test_self_loop_detected(self):
        skills = [_skill("self-ref", "desc", requires=["self-ref"])]
        errors = ssd.validate_dependency_graph(skills)
        assert errors
    ...
    def test_longer_cycle_detected(self):
        # a -> b -> c -> a
        skills = [
            _skill("a", "desc-a", requires=["b"]),
            _skill("b", "desc-b", requires=["c"]),
            _skill("c", "desc-c", requires=["a"]),
        ]
        errors = ssd.validate_dependency_graph(skills)
        assert errors

    def test_no_cycle_clean_dag(self):
        # a -> b -> c (no cycle)
        skills = [
            _skill("a", "desc-a", requires=["b"]),
            _skill("b", "desc-b", requires=["c"]),
    ...
    def test_no_requires_field_is_clean(self):
        skills = [
            _skill("a", "desc-a"),
            _skill("b", "desc-b"),
        ]
...
class TestCLI:
    ...
    def test_cli_json_contains_confusable_and_cycles_keys(self, tmp_path):
