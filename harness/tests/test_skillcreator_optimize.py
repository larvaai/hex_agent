...
_SC_SCRIPTS = (
    ...
    / "plugins" / "hs-create" / "skills" / "skill-creator" / "scripts"
)
...
def test_improve_description_no_anthropic_import():
    src = (_SC_SCRIPTS / "improve_description.py").read_text()
    assert "import anthropic" not in src
    assert "anthropic." not in src


def test_run_loop_no_html_report():
    src = (_SC_SCRIPTS / "run_loop.py").read_text()
    assert "generate_html" not in src
