...
def _hooks(repo):
    import yaml
    raw = yaml.safe_load(
        (repo / "harness" / "hooks" / "harness-hooks.yaml").read_text())
...
class TestInstallComponents:
    def test_default_honors_policy_only_spine(self, target_repo):
        ...
        # plugin groups ship OFF (default-only-hs, DEC-85), hook-components stay on.
    ...
    def test_selection_keeps_files_and_wiring(self, target_repo):
        ...
        assert (target_repo / "harness" / "hooks"
                / "decision_capture_nudge.py").is_file()
        # wire-all: deselected hook still wired in settings.json
        assert any("decision_capture_nudge.py" in c
