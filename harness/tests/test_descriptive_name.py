...
def test_generic_utils_js_flagged():
    import descriptive_name
    assert descriptive_name.core(_write("src/utils.js"))


def test_generic_temp_py_flagged():
    import descriptive_name
    assert descriptive_name.core(_write("a/b/temp.py"))
...
def test_subprocess_default_off_is_silent(tmp_path):
    ...
    assert "advisory" not in r.stderr.lower()
