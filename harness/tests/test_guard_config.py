...
def test_show_reflects_preset(tmp_path):
    ...
    r = _run(["show"], _env(tmp_path, policy))
    assert r.returncode == 0, r.stderr
    # under lenient: enforcement -> warn, advisory -> off, safety floor -> block
    lines = {ln.split()[0]: ln for ln in r.stdout.splitlines() if ln.strip()
             and ln.split()[0] in guard_policy.GUARD_REGISTRY}
    assert "warn" in lines["ownership_reconcile"]
    assert "off" in lines["memory_gap_hook"]
...
def test_set_writes_override_and_traces(tmp_path):
    ...
    r = _run(["set", "ownership_reconcile", "warn"], env)
    assert r.returncode == 0, r.stderr
    assert guard_policy.resolve_mode("ownership_reconcile", policy) == "warn"
    ...
    assert "ownership_reconcile" in changed[-1]["note"]
...
def test_header_comment_preserved_on_write(tmp_path):
    policy = _seed(tmp_path, "balanced")
    _run(["set", "ownership_reconcile", "warn"], _env(tmp_path, policy))
