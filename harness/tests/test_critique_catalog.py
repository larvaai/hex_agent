"""test_critique_catalog.py — hs-think:critique skill + agent + chain wiring.
...
_SKILLS = _HARNESS / "plugins" / "hs-think" / "skills"
...
class TestCritiqueSkillRegistration:
    def test_skill_discovered_and_owned(self):
        ...
        assert "critique" in cat["owned"]  # frontmatter name is hs-think:critique

    def test_skill_slug_maps_to_dir(self):
        cat = catalog.load_catalog(_SKILLS)
        assert cat["slug_to_dir"].get("hs-think:critique") == "critique"
...
class TestChainWiring:
    def test_plan_to_critique_chain_declared(self):
        ...
        assert ["hs:plan", "hs-think:critique"] in chains
