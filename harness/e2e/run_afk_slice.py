...
Two parts, both deterministic and offline:
  ...
  Part A — the gate spine. Feed the gate a representative loop sequence:
    1. `git commit`        -> PASS (soft stage). Freedom in the middle.
    2. `git push` (no art) -> BLOCK exit 2. The output end is gated.
    3. write verification  -> `git push` PASSES. Push opens on a presence
                              artifact (presence gate, not authn).
    4. `gh pr create`      -> still BLOCK: pr needs review-decision too.
    5. review-decision PASS_WITH_RISK -> pr STILL blocks: only an explicit PASS
                              opens a hard ship stage. The loop's own soft
                              self-accept does not ship.
    6. review-decision PASS, no plan-approval -> pr STILL blocks on the human's
...
def part_a(tmp: Path) -> None:
    ...
    proc = run("git push")
    _check("push without verification BLOCKS exit 2", proc.returncode == 2,
           "rc=%s %s" % (proc.returncode, proc.stderr[:160]))
    _check("push block names the missing verification artifact",
           "verification" in proc.stderr, proc.stderr[:160])

    # 3. push opens on a presence artifact (the loop may write its own; that is
    #    push is presence-gated, the real bar is at ship.
    ...
    _check("pr with only verification STILL BLOCKS", proc.returncode == 2,
           "rc=%s %s" % (proc.returncode, proc.stderr[:160]))
    _check("pr block names review-decision",
           "review-decision" in proc.stderr, proc.stderr[:160])
    ...
    proc = run("gh pr create --fill")
    _check("pr with PASS_WITH_RISK review STILL BLOCKS", proc.returncode == 2,
           "rc=%s %s" % (proc.returncode, proc.stderr[:160]))
    _check("block explains only exactly PASS opens a hard stage",
    ...
    proc = run("gh pr create --fill")
    _check("pr without plan-approval STILL BLOCKS (human gate at the exit)",
           proc.returncode == 2, "rc=%s %s" % (proc.returncode,
                                               proc.stderr[:160]))
    _check("pr block names plan-approval",
           "plan-approval" in proc.stderr, proc.stderr[:160])
