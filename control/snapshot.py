...
def build_snapshot(
    ...
    cp_index: dict[str, dict[str, Any]] = {}  # checkpoint_id -> its entry, so approval.* can resolve it
    ...
    for ev in events:
        ...
        if et == "loop.team_composed":
            ...
            # Resolve the gate the UI is waiting on. Without this the read-model keeps shipping
            # a 'waiting' checkpoint after the stream already delivered the approval — a phantom
            # Approval modal that contradicts the SSE stream (S21.9: snapshot must agree).
            ...
            if entry is not None:
                ...
                if created:
                    entry["resolved_at"] = str(created)
            # the agent is no longer gated once its checkpoint resolves
