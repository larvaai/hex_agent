...
# ── hardening: hostile body, cross-session write, issuer attribution ───────────
def test_post_command_non_object_body_rejected():
    """A hostile/buggy client can POST a JSON array/string/number/null. parse_command rejects
    it, but the rejection path must NOT assume the body is a dict (body.get(...)) — that
    AttributeError would crash the request thread and drop the wire response. Clean 400 ack."""
    cp = _cp()
    for bad in ([], "nope", 42, True, None):
        status, ack = cp.submit_command(token="tok", body=bad)
        assert status == 400 and ack["status"] == "rejected", f"body={bad!r}"


def test_post_command_wrong_session_rejected():
    """The read path 404s on an unknown session; the write path must not silently accept a
    command for a DIFFERENT session (dedup keyed on an unvalidated session_id is bypassable)."""
    cp = _cp()
    status, ack = cp.submit_command(token="tok", body=_cmd(session_id="someone-else"))
    assert status == 400 and ack["status"] == "rejected"
    assert "command.received" not in cp.emitted_types


def test_command_received_attributes_real_issuer():
    """The command.received audit event must carry the ACTUAL issuer, not a constant actor —
    otherwise every command looks like it came from {human, ui} and the audit trail is useless."""
    cp = _cp()
    cp.submit_command(token="tok", body=_cmd(issued_by={"type": "human", "user_id": "alice"}))
    received = [e for e in cp._emitted if e["event_type"] == "command.received"]
    assert received, "expected a command.received event"
    assert "alice" in json.dumps(received[-1])  # issuer preserved (envelope actor and/or ui_payload)
