...
# ── S21.9 — approval.* resolves the checkpoint (read-model must agree with the stream) ─
def test_build_snapshot_folds_approval_resolves_checkpoint():
    """A checkpoint reaches 'waiting', then approval.approved resolves it. The snapshot's
    checkpoint must flip to 'approved' — otherwise a finished, already-approved session still
    ships a 'waiting' checkpoint and the UI renders a phantom Approval modal that contradicts
    the SSE stream (which delivered approval.approved). The read-model must agree."""
    events = [
        _ev("loop.team_composed", {"selected": ["B"]}),
        _ev(
            "checkpoint.reached",
            {"agent_id": "B", "checkpoint_id": "cp1", "checkpoint_type": "before_tool_call",
             "risk_level": "high", "status": "waiting"},
        ),
        _ev("approval.approved", {"checkpoint_id": "cp1", "agent_id": "B"}),
        _ev("loop.turn", {"agent_id": "B", "outcome": "built it"}),
        _ev("loop.finished", {"summary": "done"}),
    ]
    snap = build_snapshot(events, session_id="s1")
    assert snap.status == "finished"
    cp = {c["checkpoint_id"]: c for c in snap.checkpoints}["cp1"]
    assert cp["status"] == "approved"
    # invariant: a finished session must carry no still-waiting checkpoint (no phantom modal)
    assert not any(c.get("status") == "waiting" for c in snap.checkpoints)


def test_build_snapshot_folds_approval_rejected():
    events = [
        _ev("checkpoint.reached", {"agent_id": "B", "checkpoint_id": "cp9", "status": "waiting"}),
        _ev("approval.rejected", {"checkpoint_id": "cp9", "agent_id": "B"}),
    ]
    snap = build_snapshot(events, session_id="s1")
    cp = {c["checkpoint_id"]: c for c in snap.checkpoints}["cp9"]
