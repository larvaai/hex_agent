...
| lens *(drag_from_zero)* | Một góc nhìn ADVISORY: 1 prompt ngắn (35B-trivial) do user viết, chạy 1 call → 1 dòng, vào `observations` của agent. Lens KHÔNG quyết — agent/code chốt (no-forge). Chạy trong `run_lenses` thuần, không cầm ToolRegistry (DEC-17/18, `dragzero/lens.py`). |
| combo *(drag_from_zero)* | Danh sách lens hữu hạn (có thứ tự), có thể chứa 1 lens "tổng hợp" đọc output lens khác (cascade). Dữ liệu config user viết (`ComboSpec`); validate acyclic lúc build. |
| hệ *(drag_from_zero)* | "Trường phái" của agent — attr trên agent node (`Agent.he`). Agent thuộc hệ X + `enabled` → CODE **ép** chạy combo của hệ X (mandate, agent không skip). Toggle `enabled` chỉ user (config frozen). |
| cascade synthesis lens *(drag_from_zero)* | Lens đọc dòng của lens khác (`reads:`) rồi nêu góc nhìn tổng hợp. Giữ propose-only: agent LUÔN nhận TẤT CẢ dòng thô + dòng tổng-hợp; tổng-hợp **append, không thay thế**. = aggregator layer của MoA nhưng advisory (DEC-18). |
