...
| **Built-in descriptor chưa phân loại đủ** | 🟡 LATENT | feature installers + `middleware/retry.py` | Retry đã hiểu `kind/idempotent`, nhưng nhiều built-in vẫn dùng descriptor mặc định `kind=tool`; policy block từ wrapper cũng chưa dùng một metadata flag thống nhất. | Trước khi bật retry cho effect: khai đúng descriptor cho mọi tool và chuẩn hóa `policy_block` metadata. |
