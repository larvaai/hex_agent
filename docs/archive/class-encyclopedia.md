...
> ⚠ **SNAPSHOT LỊCH SỬ** — tài liệu này có TRƯỚC `KernelSession` / delegation / control-plane (E21), và LangGraph schema v2. **KHÔNG phải sự thật hiện tại.** Cho runtime flow hiện tại dùng [../reference/runtime-flow.md](../reference/runtime-flow.md) + [../reference/assets/class_dependency.mermaid](../reference/assets/class_dependency.mermaid). Giữ lại như tham chiếu lịch sử.
